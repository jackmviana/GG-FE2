...
let filteredGames;
    ...
    <div className=' flex justify-center detail-bg'>
        <div ref={animationParent} className=' grid grid-cols-1 md:grid-cols-3 gap-10 p-4 transition-all ease-in-out'>
            <div className=" relative flex justify-center h-40 w-72 md:w-80 md:h-60 bg-gray-800 rounded-xl overflow-visible list-card">
            ...
            <div key={games.title}  style ={{backgroundColor: `${games.color}`}}  className=" relative h-48 w-72 md:w-80 md:h-60 bg-gray-800 grid rounded-3xl overflow-hidden game-card">
                <div className=" w-72 h-28 md:w-80 md:h-40 rounded-t-xl " style ={{backgroundImage: `url(${games.photo})`, backgroundSize: 'cover'}}>
                </div>
                <div className=' h-1/4'>
                    <p  className=" font-semibold text-center text-base md:text-xl text-gray-200 game-title" key={games.title}>{games.title}</p>
                </div>
